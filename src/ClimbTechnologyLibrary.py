...
def save_news_to_db(news_items):
    ...
    with DatabaseHandler() as cursor:
        ...
        if latest_create_time:
            logger.info(f"当前数据库最新记录时间：{latest_create_time.strftime('%Y-%m-%d')},开始更新数据...")
        # 新增步骤1 ▲▲▲
        ...
        for item in news_items:
            ...
            if cursor.fetchone():
                ...
                logger.info(f'⏩ 跳过已存在新闻 >> {item["news_title"]}：{item["news_url"]}')
                continue
            ...
            try:
                ...
                if latest_create_time:
                    ...
                    if publish_date < db_date:
                        ...
                        logger.info(f"⏩ 跳过早于数据库最新时间的新闻 >> {item['news_title']}：{item['news_url']}")
                        continue
            except ValueError as e:
                time_format_error += 1  # 统计格式错误
                logger.error(f"⚠️ 时间格式错误：{item['publish_time']} - {str(e)}")
                continue
            # 新增时间比对逻辑 ▲▲▲
...
def parse_news_content(news_url):
    if not news_url.startswith('http'):
        logger.error(f"无效的新闻链接：{news_url}")
        return ''
    ...
    try:
        if not check_robots_permission(news_url):
            logger.error(f"⛔ 禁止爬取详情页：{news_url}")
            return ''
        ...
        logger.error(f"详情页解析异常：{news_url} - {str(e)}")
    return ''
...
def parse_page(html):
    ...
    for item in news_items:
        logger.info(f"{item.get('publish_time', 'N/A')}:{item.get('news_title', '')}")
...
def main():
    ...
    if not check_robots_permission(base_domain):
        logger.error(f'⛔ 根据 robots.txt 协议禁止爬取 {base_domain}')
        exit(1)
    ...
    for page_number, url in enumerate(urls, start=1):
        ...
        try:
            if not check_robots_permission(url):
                logger.error(f'⛔ 根据 robots.txt 协议禁止爬取 {url}')
                continue  # 跳过被禁止的URL
        except Exception as e:
            logger.error(f'⚠️ robots.txt检测失败：{str(e)}，继续执行爬取')

        # 原有爬取逻辑
        if html := get_html_text(url):
            page_news = parse_page(html)
            logger.info(f'第{page_number}页抓取到{len(page_news)}条新闻')
